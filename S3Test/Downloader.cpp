...
void FileDownloader::download(const fs::path& fully_qualified_name, const fs::path& target_file)
{
    ...
    downloading_thread = std::thread([this, fqn{fully_qualified_name}] { downloadThread(fqn); });

    std::ofstream file(target_file, std::ios::binary | std::ios::trunc);
    auto begin = std::chrono::high_resolution_clock::now();

    while((!part_completion.empty() || has_work_to_do) && !abort)
    {
        Aws::S3::Model::GetObjectResult part;
        {
            {
                std::unique_lock<std::mutex> lck(download_mtx);
                if(!cv.wait_for(lck, 1ms, [this] { return ((!part_completion.empty() || has_work_to_do) && !abort); }))
                    break;
            }
            while(!part_completion.empty())
            {
                ...
                std::cout << "Part downloaded" << std::endl;
                cv.notify_all();
            }
        }
    }
...
void FileDownloader::downloadThread(const fs::path& fully_qualified_name)
{
    S3File s3file(fully_qualified_name);
    size_t file_size = s3file.size();
