...
class pool
{
	...
	enum pool_defaults
	{
		init_size = 0xfffff, min_size = 0xf
	};
public:
	pool(
		...
		size_t size)
	{
		...
		{
			block * new_block =
				(reinterpret_cast<block *>(reinterpret_cast<char *>(b)
				+ size + sizeof(block)));
			if (b->next_)
				b->next_->prev_ = new_block;
		...
		size_t = 0)
	{
		if (!p)
			return;
		block *b = reinterpret_cast<block *>(static_cast<char*>(p)
			- sizeof(block));
		if (b->prev_ && b->next_)
		{
			if (b->prev_->free_ && b->next_->free_)
			{
				b->prev_->size_ += b->size_ + b->next_->size_
					+ 2 * sizeof(block);
				b->prev_->next_ = b->next_->next_;
				if (b->next_->next_)
					b->next_->next_->prev_ = b->prev_;
				return;
			}
		}
		if (b->prev_)
