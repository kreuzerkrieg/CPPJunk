...
static void call_with_instrinsics(std::span<std::uint64_t> a, std::span<std::uint64_t> b, std::span<std::uint64_t> res)
{
	eve::algo::transform_to(eve::views::zip(a, b), res, [](auto in) {
		auto [wide_a, wide_b] = in;
		return eve::wide<std::uint64_t>{_mm_clmulepi64_si128(wide_a, wide_b, 0)};
	});
}
