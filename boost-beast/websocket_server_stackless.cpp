...
#include <boost/asio/yield.hpp>
	...
	void loop(boost::system::error_code ec, std::size_t bytes_transferred)
	{
		...
		reenter(*this)
		{
			...
			yield
			ws_.async_accept(boost::asio::bind_executor(strand_, std::bind(&session::loop, shared_from_this(), std::placeholders::_1, 0)));
			if (ec) {
			...
			for (;;) {
				...
				ws_.async_read(buffer_, boost::asio::bind_executor(strand_,
																   std::bind(&session::loop, shared_from_this(), std::placeholders::_1,
																			 std::placeholders::_2)));
				...
				yield
				ws_.async_write(buffer_.data(), boost::asio::bind_executor(strand_, std::bind(&session::loop, shared_from_this(),
																							  std::placeholders::_1,
																							  std::placeholders::_2)));
				if (ec) {
...
#include <boost/asio/yield.hpp>
	...
	void loop(boost::system::error_code ec = {})
	{
		reenter(*this)
		{
			for (;;) {
				yield
				acceptor_.async_accept(socket_, std::bind(&listener::loop, shared_from_this(), std::placeholders::_1));
				if (ec) {
					fail(ec, "accept");
				}
				else {
...
int main(int argc, char* argv[])
{
	...
	if (argc != 4) {
		std::cerr << "Usage: websocket-server-stackless <address> <port> <threads>\n" << "Example:\n"
