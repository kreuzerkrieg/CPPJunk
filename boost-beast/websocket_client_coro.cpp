...
void do_session(std::string const& host, std::string const& port, std::string const& text, boost::asio::io_context& ioc,
				boost::asio::yield_context yield)
{
	...
	auto delta = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count();
	std::cout << double(cycles) / delta * 1'000'000'000 << "IOPS" << std::endl;
...
int main(int argc, char** argv)
{
	...
	std::string text(512, 'c');

	// The io_context is required for all I/O
	boost::asio::io_context ioc;

	// Launch the asynchronous operation
	boost::asio::spawn(ioc, std::bind(&do_session, std::string(host), std::string(port), text, std::ref(ioc), std::placeholders::_1));
